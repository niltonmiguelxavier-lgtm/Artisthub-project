    ...
    <div>
      <svg viewBox={`0 0 100 ${height}`} preserveAspectRatio="none" className="w-full" style={{ height }}>
        {data.map((point, i) => {
          ...
          return (
            <g key={point.month}>
              <rect x={x} y={y} width={w} height={barHeight} rx={1.5} fill="#3B7DD8" opacity={i === data.length - 1 ? 1 : 0.55} />
            </g>
          );
        })}
      </svg>
      ...
      <p className="mt-3 text-xs text-bone-400">
        Último mês: <span className="font-mono-data text-cobalt-400">{formatCurrency(data[data.length - 1]?.amount ?? 0)}</span>
      </p>
    </div>
  );
}
