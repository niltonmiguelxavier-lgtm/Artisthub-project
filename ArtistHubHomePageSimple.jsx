...
const NAV_LINKS = ["Início", "Explorar", "Artistas", "Lançamentos", "Vídeos"];
...
const FEATURED_ARTISTS = [
  { id: 1, name: "Nayara Mbeve", genre: "Marrabenta Soul", verified: true },
  { id: 2, name: "Nilton Xavier", genre: "R&B Lusófono", verified: true },
  { id: 3, name: "Zeca Nhampossa", genre: "Afrobeat", verified: true },
];
...
function CoverArt({ icon: Icon = Music2, rounded = "rounded-xl", className = "" }) {
  return (
...
function PortraitArt({ className = "" }) {
  return (
...
function SectionHeader({ title, subtitle, action }) {
  return (
    <div className="flex items-end justify-between mb-7 gap-4">
      ...
      {action && (
        <a href="#" className="hidden sm:inline-flex items-center gap-1 text-sm font-medium shrink-0" style={{ color: INK }}>
    ...
    <header className="sticky top-0 z-40 backdrop-blur border-b" style={{ background: `${BG}F5`, borderColor: BORDER }}>
      <div className="max-w-6xl mx-auto px-5 h-16 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-7">
          {NAV_LINKS.map((l) => (
            <a key={l} href="#" className="text-sm font-medium transition-colors" style={{ color: MUTED }}>
              {l}
            </a>
          ))}
        </nav>
        ...
        <div className="flex items-center gap-2.5">
          <button className="p-2 rounded-full hover:bg-black/[0.04] transition-colors" style={{ color: MUTED }} aria-label="Pesquisar">
            <Search className="w-4 h-4" />
          </button>
          <button className="hidden sm:block px-4 py-2 text-sm font-medium rounded-full border transition-colors" style={{ color: INK, borderColor: BORDER }}>
            Entrar
          </button>
          <button className="px-4 py-2 text-sm rounded-full font-medium text-white transition-colors" style={{ background: INK }}>
            Criar conta
          </button>
        </div>
      </div>
        ...
        <div className="relative grid md:grid-cols-[1.05fr_0.95fr] gap-12 items-center">
          <div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              <button className="inline-flex items-center gap-2 px-6 py-3 rounded-full font-medium" style={{ background: ACCENT, color: INK }}>
                <Play className="w-4 h-4" fill="currentColor" /> Explorar música
              </button>
              <button className="inline-flex items-center gap-2 px-6 py-3 rounded-full font-medium border border-white/15 text-white">
...
function RecentReleases({ onPlay, playingId }) {
  return (
    <section className="max-w-6xl mx-auto px-5 py-14">
      <SectionHeader title="Lançamentos recentes" subtitle="As músicas que acabaram de chegar ao ArtistHub." action="Ver todos" />
...
function TrendingRanking({ onPlay, playingId }) {
  return (
...
function FeaturedArtists() {
  return (
    <section className="max-w-6xl mx-auto px-5 py-14">
      <SectionHeader title="Artistas em destaque" subtitle="Vozes que estão a definir o som lusófono." action="Ver diretório" />
        ...
        {FEATURED_ARTISTS.map((a) => (
          <div key={a.id} className="rounded-2xl overflow-hidden border transition-shadow hover:shadow-lg" style={{ borderColor: BORDER, background: CARD, boxShadow: SHADOW }}>
            ...
            <div className="p-4 flex items-center justify-between">
              ...
              <button className="shrink-0 px-3.5 py-1.5 text-xs font-semibold rounded-full text-white transition-colors" style={{ background: INK }}>
                Seguir
              </button>
            </div>
          </div>
...
function NewTalent() {
  return (
    <section className="max-w-6xl mx-auto px-5 py-4">
      <SectionHeader title="Novos talentos" subtitle="Descobre artistas que estão a começar a conquistar espaço no ArtistHub." action="Explorar artistas" />
          ...
          {SUPPORT_ARTISTS.map((a) => (
            <div key={a.id} className="flex items-center gap-3 rounded-xl p-3 border" style={{ borderColor: BORDER, background: BG }}>
              ...
              <div className="min-w-0 flex-1">
                ...
                <button className="text-[11px] font-semibold mt-0.5" style={{ color: ACCENT }}>Apoiar</button>
              </div>
            </div>
          ))}
        </div>

        <button className="px-6 py-3 rounded-full font-medium" style={{ background: ACCENT, color: INK }}>
          Apoiar artistas
        </button>
      </div>
    </section>
  ...
  return (
    <section className="max-w-6xl mx-auto px-5 py-4">
      <div className="rounded-3xl p-8 sm:p-12 flex flex-col sm:flex-row items-center justify-between gap-6" style={{ background: INK }}>
        ...
        <button className="shrink-0 px-7 py-3.5 rounded-full font-medium" style={{ background: ACCENT, color: INK }}>
      ...
      <div className="rounded-3xl border p-6 sm:p-10 grid sm:grid-cols-[auto_1fr] gap-8 items-center" style={{ borderColor: BORDER, background: CARD, boxShadow: SHADOW }}>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-3">
            <button className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full font-medium text-white" style={{ background: INK }}>
              <Play className="w-4 h-4" fill="currentColor" /> Ouvir
            </button>
            <button className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full font-medium border" style={{ color: INK, borderColor: BORDER }}>
...
function PlayerBar({ playingId, onTogglePlay }) {
...
export default function ArtistHubHomePageSimple() {
